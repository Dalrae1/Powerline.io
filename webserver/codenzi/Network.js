var Network = function () {
  ...
  this.getServerAndConnect = function () {
    ...
    if (!inIframe()) locationHash = parent.location.hash;
  ...
  this.connect = function () {
    ...
    if (isSecure) {
      ...
      if (network.roomNumber > 0) {
        ...
        fullhost =
          "ws:" + addrSplit[1] + ":" + (parseInt(network.roomNumber) + 8080);
  ...
  function CreateEntity(type, subType) {
    ...
    switch (type) {
      ...
      case ENTITY_ITEM: {
        if (subType == SUB_ENTITY_ITEM_FOOD) entity = new Food();
        else if (subType == SUB_ENTITY_ITEM_ENERGY) entity = new Energy();
        else if (
          subType == SUB_ENTITY_ITEM_TRI_PLUS ||
          subType == SUB_ENTITY_ITEM_TRI_MINUS
        )
  ...
  function updateEntities(view) {
    ...
    while (true) {
      ...
      switch (flags) {
        case 0x0: // Partial
          entity = entities[id];
          if (entity) offset = entity.updateNetwork(view, offset, false);
          else console.log("entity with id: " + id + " not found"); //debug.log();
          break;
        case 0x1: // Full
          ...
          if (entity) {
            ...
            console.log(
              "Unable to create entity. Entity Type is: " + entityType
            );
          }
          break;
        case 0x2: // Delete
  ...
  this.processConfig = function (view, op) {
    ...
    if (op == OPCODE_CONFIG_2) hasSmartResize = true;
    ...
    if (debug) console.log(arenaWidth / GAME_SCALE);
    ...
    if (globalWebLag == 0) antiLagEnabled = false;
    else antiLagEnabled = true;
  ...
  this.processMessage = function (data) {
    ...
    if (op == OPCODE_SC_PONG) {
      if (statsVisible) {
        ...
        if (statsLAG) statsLAG.updateLag(pingTime);
      ...
      if (id > 0) {
        ...
        if (statLength == 0 || statLength < score) statLength = score;
  ...
  this.sendHello = function () {
    ...
    var view = new DataView(buf);
    if (debug) view.setUint8(0, OPCODE_HELLO_DEBUG);
    else view.setUint8(0, OPCODE_HELLO_V4);
  ...
  this.sendTurnPoint = function (direction, coord) {
    ...
    var flags = 0x0;
    if (!focus || UIVisible) flags = flags | 0x1; // Paused ?
  ...
  this.sendBoost = function (boosting) {
    ...
    if (boosting) view.setUint8(1, 0x1);
